...
class GameView(ctx:Context):View(ctx),Choreographer.FrameCallback{
 ...
 private fun play(dt:Float){
  ...
  if(buffer>0&&(grounded||coyote>0)){vy=-900f;grounded=false;coyote=0f;buffer=0f;burst(x+27,y+72,0xffffd54f.toInt());beep(760)}
  ...
  for(c in cs)if(!c.got){c.a+=dt*5;if(RectF(x+7,y+7,x+pw-7,y+ph-5).contains(c.x,c.y)){c.got=true;coin++;score+=100;burst(c.x,c.y,0xffffd54f.toInt());beep(880)}}
  for(b in bs)if(hit(RectF(x,y,x+pw,y+ph),RectF(b.x,b.y,b.x+58,b.y+58))&&vy<0&&y<b.y+58){y=b.y+58;vy=80f;if(b.q){score+=150;msg="BONUS!";msgT=1f;burst(b.x+29,b.y,0xffffd54f.toInt());beep(1040)}}
  for(e in es)if(e.alive){e.x+=e.vx*dt;val support=ps.firstOrNull{e.x+46>it.x&&e.x<it.x+it.w&&abs(e.y+42-it.y)<8};if(support!=null)e.y=support.y-42 else if(e.y<ground-30)e.y+=2200*dt;if(support!=null&&(e.x<support.x||e.x+46>support.x+support.w))e.vx*=-1;if(hit(RectF(x+7,y+8,x+pw-7,y+ph-3),RectF(e.x,e.y,e.x+46,e.y+42))){if(vy>250&&y+ph<e.y+30){e.alive=false;vy=-650f;score+=250;burst(e.x+23,e.y,0xffff8a4c.toInt());beep(420)}else if(inv<=0){lose();return}}}
 ...
 private fun sky(c:Canvas){paint.shader=LinearGradient(0f,0f,0f,h,Color.rgb(105,195,255),Color.rgb(224,247,255),Shader.TileMode.CLAMP);c.drawRect(0f,0f,w,h,paint);paint.shader=null;paint.color=0x55ffffff.toInt();for(i in 0..7){val cx=(i*310f-cam*.08f)%(w+300)-120;c.drawCircle(cx,110f+(i%2)*70,42f,paint);c.drawCircle(cx+45,112f+(i%2)*70,32f,paint)};paint.color=0xff78c86b.toInt();for(i in -2..9){val xx=i*220f-cam*.12f;val path=Path();path.moveTo(xx,h*.78f+100);path.quadTo(xx+100,h*.68f-80,xx+220,h*.78f+100);path.close();c.drawPath(path,paint)}}
 ...
 private fun menu(c:Canvas){sky(c);txt.color=0xff18202a.toInt();txt.textSize=min(w*.14f,76f);c.drawText("SUPER RUN",w/2,h*.24f,txt);txt.textSize=20f;c.drawText("BEST  "+best,w/2,h*.31f,txt);button(c,"PLAY",w*.22f,h*.40f,w*.78f,h*.50f);button(c,"HOW TO PLAY",w*.22f,h*.54f,w*.78f,h*.64f);button(c,"SOUND: "+if(sound)"ON" else "OFF",w*.22f,h*.68f,w*.48f,h*.77f);button(c,"VIBRATION: "+if(vib)"ON" else "OFF",w*.52f,h*.68f,w*.78f,h*.77f);txt.textSize=14f;c.drawText("Run • jump • collect • stomp • reach the flag",w/2,h*.86f,txt)}
 private fun how(c:Canvas){sky(c);txt.color=0xff18202a.toInt();txt.textSize=34f;c.drawText("HOW TO PLAY",w/2,h*.18f,txt);txt.textSize=19f;listOf("◀ / ▶  Move","JUMP  Tap or hold for a higher jump","Collect coins for points","Stomp enemies from above","Avoid gaps and reach the finish flag","Phone BACK pauses / returns to menu").forEachIndexed{i,v->c.drawText(v,w/2,h*(.31f+i*.075f),txt)};button(c,"BACK",w*.30f,h*.78f,w*.70f,h*.88f)}
 private fun game(c:Canvas){sky(c);c.save();c.translate(-cam,0f);for(q in ps){paint.color=0xff7b4f2c.toInt();c.drawRect(q.x,q.y,q.x+q.w,q.y+q.h,paint);paint.color=0xff4caf50.toInt();c.drawRect(q.x,q.y,q.x+q.w,q.y+9,paint)};val fx=end-300;paint.color=0xff6d4c41.toInt();c.drawRect(fx,ground-250,fx+12,ground,paint);paint.color=0xffef5350.toInt();val flag=Path();flag.moveTo(fx+12,ground-245);flag.lineTo(fx+105,ground-215);flag.lineTo(fx+12,ground-180);flag.close();c.drawPath(flag,paint);for(b in bs){paint.color=if(b.q)0xffffb52e.toInt() else 0xff9b6a3c.toInt();c.drawRoundRect(b.x,b.y,b.x+58,b.y+58,7f,7f,paint);if(b.q){txt.color=Color.WHITE;txt.textSize=34f;c.drawText("?",b.x+29,b.y+40,txt)}};for(co in cs)if(!co.got){paint.color=0xffffd54f.toInt();c.drawCircle(co.x,co.y,13f+2f*sin(co.a),paint)};for(e in es)if(e.alive){paint.color=0xff7b3f24.toInt();c.drawRoundRect(e.x,e.y,e.x+46,e.y+42,12f,12f,paint);paint.color=Color.WHITE;c.drawCircle(e.x+14,e.y+15,7f,paint);c.drawCircle(e.x+32,e.y+15,7f,paint);paint.color=Color.BLACK;c.drawCircle(e.x+14,e.y+15,3f,paint);c.drawCircle(e.x+32,e.y+15,3f,paint)};if(!(inv>0&&(inv*12).toInt()%2==0)){val bob=if(grounded)sin(anim*8)*2 else 0f;paint.color=0xffe53935.toInt();c.drawRoundRect(x+8,y+18+bob,x+pw-8,y+ph-18,13f,13f,paint);paint.color=0xffffc49b.toInt();c.drawCircle(x+pw/2,y+17+bob,20f,paint);paint.color=0xffb71c1c.toInt();c.drawRoundRect(x+8,y+2+bob,x+pw-6,y+16+bob,10f,10f,paint);paint.color=0xff1565c0.toInt();c.drawRect(x+10,y+ph-25+bob,x+pw/2-2,y+ph-2+bob,paint);c.drawRect(x+pw/2+2,y+ph-25+bob,x+pw-10,y+ph-2+bob,paint)};for(f in fs){paint.color=f.color;c.drawCircle(f.x,f.y,max(1f,f.life*5),paint)};c.restore();paint.color=0xaaffffff.toInt();c.drawRoundRect(12f,12f,w-12,66f,18f,18f,paint);txt.color=0xff18202a.toInt();txt.textSize=17f;c.drawText("SCORE "+score,90f,46f,txt);c.drawText("COINS "+coin,w*.34f,46f,txt);c.drawText("LIVES "+lives,w*.57f,46f,txt);c.drawText("TIME "+timer.toInt(),w-70f,46f,txt);button(c,"◀",24f,h-112,112f,h-24);button(c,"▶",128f,h-112,216f,h-24);button(c,"JUMP",w-122f,h-124,w-20f,h-34);if(msgT>0){txt.color=Color.WHITE;txt.textSize=32f;c.drawText(msg,w/2,h*.22f,txt)}}
 private fun hit(a:RectF,b:RectF)=a.left<b.right&&a.right>b.left&&a.top<b.bottom&&a.bottom>b.top
 private fun button(c:Canvas,label:String,l:Float,t:Float,r:Float,b:Float){paint.color=0xccffffff.toInt();c.drawRoundRect(l,t,r,b,18f,18f,paint);txt.color=0xff18202a.toInt();txt.textSize=if(label.length>6)15f else 24f;c.drawText(label,(l+r)/2,(t+b)/2-txt.ascent()/2,txt)}
 private fun overlay(c:Canvas,a:String,b:String){paint.color=0x99000000.toInt();c.drawRect(0f,0f,w,h,paint);txt.color=Color.WHITE;txt.textSize=min(w*.09f,54f);c.drawText(a,w/2,h*.39f,txt);txt.textSize=20f;c.drawText(b,w/2,h*.49f,txt);txt.textSize=18f;c.drawText("BEST "+best,w/2,h*.57f,txt)}
