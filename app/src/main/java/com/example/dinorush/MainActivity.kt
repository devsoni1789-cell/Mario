...
class MainActivity : AppCompatActivity(), GameView.Listener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        wireMenu(); wirePause(); wireGameOver(); wireSettings(); wireHowToPlay(); wireHighScores(); wireHud()
        showScreen(Screen.SPLASH)
        mainHandler.postDelayed({ if (currentScreen == Screen.SPLASH) showScreen(Screen.MENU) }, 1800)
    ...
    private fun showScreen(screen: Screen) {
        ...
        if (screen == Screen.PLAYING) onScreenControls.visibility = if (prefs.onScreenControls) View.VISIBLE else View.GONE
    ...
    private fun handleBack() {
        when (currentScreen) {
            Screen.SPLASH -> {}
            Screen.PLAYING -> { gameView.pauseGame(); showScreen(Screen.PAUSED) }
            Screen.PAUSED, Screen.GAME_OVER -> { gameView.stopToMenu(); showScreen(Screen.MENU) }
            Screen.SETTINGS, Screen.HOW_TO_PLAY, Screen.HIGH_SCORES -> showScreen(Screen.MENU)
            Screen.MENU -> {
                val now = System.currentTimeMillis()
                if (now - backPressedOnceAt < 2000) finish()
                else {
    ...
    private fun wireMenu() {
        findViewById<Button>(R.id.btnPlay).setOnClickListener { soundManager.playClick(); gameView.startNewGame(); showScreen(Screen.PLAYING) }
        findViewById<Button>(R.id.btnHighScores).setOnClickListener { soundManager.playClick(); showScreen(Screen.HIGH_SCORES) }
        findViewById<Button>(R.id.btnSettings).setOnClickListener { soundManager.playClick(); loadSettingsIntoUi(); showScreen(Screen.SETTINGS) }
        findViewById<Button>(R.id.btnHowToPlay).setOnClickListener { soundManager.playClick(); showScreen(Screen.HOW_TO_PLAY) }
        findViewById<Button>(R.id.btnExit).setOnClickListener { soundManager.playClick(); finish() }
    }

    private fun wirePause() {
        findViewById<ImageButton>(R.id.btnPause).setOnClickListener { soundManager.playClick(); gameView.pauseGame(); showScreen(Screen.PAUSED) }
        findViewById<Button>(R.id.btnResume).setOnClickListener { soundManager.playClick(); gameView.resumeGame(); showScreen(Screen.PLAYING) }
        findViewById<Button>(R.id.btnPauseRestart).setOnClickListener { soundManager.playClick(); gameView.startNewGame(); showScreen(Screen.PLAYING) }
        findViewById<Button>(R.id.btnPauseMainMenu).setOnClickListener { soundManager.playClick(); gameView.stopToMenu(); showScreen(Screen.MENU) }
    }

    private fun wireGameOver() {
        findViewById<Button>(R.id.btnGoRestart).setOnClickListener { soundManager.playClick(); gameView.startNewGame(); showScreen(Screen.PLAYING) }
        findViewById<Button>(R.id.btnGoMainMenu).setOnClickListener { soundManager.playClick(); gameView.stopToMenu(); showScreen(Screen.MENU) }
    }
    ...
    private fun wireHud() {
        findViewById<Button>(R.id.btnOnScreenJump).setOnTouchListener { _, event ->
            if (event.action == android.view.MotionEvent.ACTION_DOWN) gameView.performJump()
            true
        }
    ...
    override fun onHudUpdate(score: Int, best: Int, distanceMeters: Int, coins: Int, powerUpLabel: String?, powerUpFraction: Float) {
        runOnUiThread {
            ...
            if (powerUpLabel != null) { hudPowerUp.visibility = View.VISIBLE; hudPowerUp.text = powerUpLabel }
            else hudPowerUp.visibility = View.GONE
        }
    }
    ...
    private fun wireSettings() {
        ...
        switchMusic.setOnCheckedChangeListener { _: CompoundButton, checked: Boolean -> prefs.musicEnabled = checked; soundManager.refreshMusicState() }
        switchVibration.setOnCheckedChangeListener { _: CompoundButton, checked: Boolean -> prefs.vibrationEnabled = checked }
        switchControls.setOnCheckedChangeListener { _: CompoundButton, checked: Boolean -> prefs.onScreenControls = checked; onScreenControls.visibility = if (checked && currentScreen == Screen.PLAYING) View.VISIBLE else View.GONE }
        radioGroup.setOnCheckedChangeListener { _: RadioGroup, checkedId: Int ->
            prefs.graphicsQuality = when (checkedId) { R.id.radioLow -> 0; R.id.radioHigh -> 2; else -> 1 }
        }
        findViewById<Button>(R.id.btnResetScores).setOnClickListener {
            AlertDialog.Builder(this).setTitle(R.string.reset_confirm_title).setMessage(R.string.reset_confirm_body)
                .setPositiveButton("Reset") { _, _ -> prefs.resetHighScores(); Toast.makeText(this, "High scores reset", Toast.LENGTH_SHORT).show() }
                .setNegativeButton("Cancel", null).show()
        }
        findViewById<Button>(R.id.btnAbout).setOnClickListener {
            AlertDialog.Builder(this).setTitle(R.string.btn_about).setMessage(R.string.about_body).setPositiveButton("OK", null).show()
        }
        findViewById<Button>(R.id.btnSettingsBack).setOnClickListener { soundManager.playClick(); showScreen(Screen.MENU) }
    }
    ...
    private fun loadSettingsIntoUi() {
        ...
        findViewById<RadioGroup>(R.id.radioGraphics).check(when (prefs.graphicsQuality) { 0 -> R.id.radioLow; 2 -> R.id.radioHigh; else -> R.id.radioNormal })
    }

    private fun wireHowToPlay() {
        findViewById<Button>(R.id.btnHowToBack).setOnClickListener { soundManager.playClick(); showScreen(Screen.MENU) }
    }

    private fun wireHighScores() {
        findViewById<Button>(R.id.btnHighScoresBack).setOnClickListener { soundManager.playClick(); showScreen(Screen.MENU) }
    }
    ...
    override fun onResume() { super.onResume(); soundManager.refreshMusicState() }
    override fun onPause() {
        super.onPause()
        if (currentScreen == Screen.PLAYING) { gameView.pauseGame(); showScreen(Screen.PAUSED) }
